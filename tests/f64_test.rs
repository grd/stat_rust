...
fn test_float64_slice() {
    ...
    let vec_a = vec![0.0421, 0.0941, 0.1064, 0.0242, 0.1331, 0.0773, 0.0243, 0.0815, 0.1186,
                     0.0356, 0.0728, 0.0999, 0.0614, 0.0479];

    let vec_b = vec![0.1081, 0.0986, 0.1566, 0.1961, 0.1125, 0.1942, 0.1079, 0.1021, 0.1583,
                     0.1673, 0.1675, 0.1856, 0.1688, 0.1512];

    let vec_w = vec![0.000, 0.000, 0.000, 3.000, 0.0000, 1.000, 1.000, 1.000, 0.000, 0.5000,
    ...
    {
        let mean = mean(&vec_a);
        let varc = variance_with_fixed_mean(&vec_a, mean);
    ...
    {
        let (_, min_index, _, max_index) = minmax(&vec_a);
    ...
    let mut vec_ac = vec_a.clone();
    vec_ac[3] = f64::NAN;

    {
        let (max, max_index) = max(&vec_ac);
    ...
    {
        let (min, min_index, max, max_index) = minmax(&vec_ac);
